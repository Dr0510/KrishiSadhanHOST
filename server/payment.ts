...
export async function generateReceipt(bookingId: number, paymentId: string) {
  try {
    ...
    const equipment = booking
      ? await storage.getEquipment(booking.equipmentId)
      : null;
    ...
    currentY += lineHeight;
    doc.text(
      `Payment Date: ${new Date(payment.created_at * 1000).toLocaleString()}`,
      20,
      currentY,
    );
    currentY += lineHeight;
    ...
    currentY += lineHeight;
    doc.text(
      `Booking Period: ${new Date(booking.startDate).toLocaleDateString()} to ${new Date(booking.endDate).toLocaleDateString()}`,
      20,
      currentY,
    );
    currentY += lineHeight;
    doc.text(
      `Daily Rate: ₹${equipment.dailyRate.toLocaleString("hi-IN")}`,
      20,
      currentY,
    );
    currentY += lineHeight * 2;
    ...
    currentY += lineHeight;
    doc.text(
      `Equipment Rental: ${formatAmount(dailyRate)} × ${daysRented} = ${formatAmount(totalCalculated)}`,
      20,
      currentY,
    );
    currentY += lineHeight;
    ...
    doc.text(
      `Total Amount Paid: ${formatAmount(amountInRupees)}`,
      20,
      currentY,
    );
    doc.text(
